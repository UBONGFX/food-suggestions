...
export default function ListDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Check if the list exists and get its name
  const [listExists, setListExists] = useState<boolean | null>(null);
  const [listName, setListName] = useState<string>("");
  
  useEffect(() => {
    // Check if the list exists in localStorage and get its name
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("user-entities");
        if (saved) {
          const entities = JSON.parse(saved);
          const foundEntity = entities.find((entity: any) => entity.id === resolvedParams.id);
          if (foundEntity) {
            setListExists(true);
            setListName(foundEntity.name);
          } else {
            setListExists(false);
          }
        } else {
          // Check if it's one of the default entities (fallback)
          const defaultEntities = [
            { id: "1", name: "Familienküche" },
            { id: "2", name: "WG Küche" }
          ];
          const defaultEntity = defaultEntities.find(e => e.id === resolvedParams.id);
          if (defaultEntity) {
            setListExists(true);
            setListName(defaultEntity.name);
          } else {
            setListExists(false);
          }
        }
  ...
  // Show loading state while checking if list exists
  if (listExists === null) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
          <p className="text-lg font-medium text-gray-700 dark:text-gray-300">Loading...</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">Überprüfe Liste...</p>
        </div>
      </div>
    );
  }
      ...
      <header className="sticky top-0 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-zinc-900/60 border-b border-black/5 dark:border-white/10 z-20">
        <div className="mx-auto max-w-5xl px-6 py-4 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between gap-2">
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight leading-tight break-words flex-1 pr-4">
              🍽️ {listName || "Essensvorschläge & Planer"}
            </h1>
            {/* Desktop: nav + settings together */}
            <div className="hidden md:flex items-center gap-4">
              ...
              <Link
                href={`/lists/${resolvedParams.id}/settings`}
                className="inline-flex items-center justify-center h-9 px-3 shrink-0 rounded-md border border-black/10 dark:border-white/10 bg-white/80 dark:bg-zinc-800/80 hover:bg-black/5 dark:hover:bg-white/10 text-sm font-medium transition-colors"
          ...
          {/* Mobile: Breadcrumb navigation and settings */}
